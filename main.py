from pyscript import display 
#Restaurant Ordering System using Python Data types 

#String Data
restaurant_name = "Yummy Food"
owner_name = "Gabriella Marie Buhain"

#Integer Data 
year_since = 2025 

#Float Data
tax_rate = 0.08 

#Boolean Data
has_delivery = True
is_dine_in = True 
is_takeaway = False

#List Data 
product_names = ["cold spaghetti", "cinnamon butterscotch pie", "chocolate bar"]
beverages = ["water", "sparkling water"]

#Tuple Data
business_hours = ("1:00 AM", "11:59 PM")

#Dictionary Data
menu = {
    "cold spaghetti": 100.00,
    "cinnamon butterscotch pie": 65.00,
    "chocolate bar": 150.00,
    "water": 50.00,
    "sparkling water": .00
}

#Set Data Types 
common_allergens = {"humans", "dairy", "nuts"}

#Displaying restaurant Information
display(restaurant_name, target="name1")
display(f"Owner: {owner_name}", target="owner")
display(f"Since {year_since}", target="since")
display(f"Menu Pricelist:", target="heading1") 

#Display Menu
display(product_names[0], target="prod1")
display(f"₱{menu['cold spaghetti']: .2f}", target="price1")
display(product_names[1], target="prod2")
display(f"₱{menu['cinnamon butterscotch pie']: .2f}", target="price2")
display(product_names[2], target="prod3")
display(f"₱{menu['chocolate bar']: .2f}", target="price3")
display(beverages[0], target="prod4")
display(f"₱{menu['water']: .2f}", target="price4")
display(beverages[1], target="prod5")
display(f"₱{menu['sparkling water']: .2f}", target="price5")

#Display additional info 
display(f"Open: {business_hours[0]} - {business_hours[1]}", target= "openingHours")

#display order type
display(f"Dine-in Available", target="orderType")